...
- **互动**：
  ...
  - 右键：完整菜单（模式 / 大小 / 设置 ▸ Key·城市·监控间隔 / 测试DS连接 / 聊天记录 / 天气 / 开关项 / 窗口项 / 退出；托盘右键同款，穿透后可从托盘解除）
- **台词系统**：日常随机台词 + 互动回嘴 + 思维链心声（灰色斜体括号气泡），取材自社区 DS 梗
- **细节**：呼吸 / 摇摆 / 蹦跳 / 进食动画、转向交叉淡化、加减速惯性、散步自动休息、说话冷却
- **AI 对话**：调用 DeepSeek API（`deepseek-v4-flash`，新端点无 `/v1` 前缀），流式回复逐段显示；风格贱兮兮但可爱（超长回复不截断、气泡自动换行并按字数延长停留时间）；历史持久化（重启恢复，右键「聊天记录」可回看/清空）；429/5xx 自动指数退避重试；Key 经「设置 ▸ 设置 Key」输入（密文回显，保存后即时验证），存于本地 `config.json`（已 gitignore，勿分享）；「深度思考」可在右键菜单开关（开启后回复更聪明但更慢，超时放宽到 60 秒）；等待回复时头顶显示 💭 思考气泡；连接复用降低延迟；「使用系统代理」可开关（代理拦截导致超时时关闭走直连，对天气查询同样生效）
- **天气查询**：右键菜单「查看天气」→ wttr.in，城市可经「设置 ▸ 设置城市」修改
- **系统监控**（默认关闭，右键菜单「系统监控」开启）：CPU > 90% / 内存 > 95% / NVIDIA 显卡（多卡取最高温）> 80°C 时气泡提醒；检测间隔可经「设置 ▸ 监控间隔」手动修改（默认 10 秒，范围 5~3600 秒），配置存 `config.json`（`monitor_enabled` / `monitor_interval_s`）
