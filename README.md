...
- **AI 对话**：调用 DeepSeek API（`deepseek-v4-flash`，新端点无 `/v1` 前缀），每句不超过 25 字，风格贱兮兮但可爱；历史保留最近 40 条；Key 经右键菜单「设置 Key」输入（密文回显），存于本地 `config.json`（已 gitignore，勿分享）；「深度思考」可在右键菜单开关（开启后回复更聪明但更慢，超时放宽到 60 秒）
- **天气查询**：右键菜单「查看天气」→ wttr.in，城市可经「设置城市」修改
- **系统监控**：CPU > 90% / 内存 > 95% / NVIDIA 显卡 > 80°C 时气泡提醒（10 秒检测一次）
